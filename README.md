...
Settings (defaults follow subtitle-industry standards): `words` per line (1-3), `pause` (a silence this long starts a new line), `hang` (how long a line outlives its last word), `min` (minimum time on screen), `gap` (empty space before the next line, default 0; all in seconds), `punctuation keep|remove`, and `silence hold|cut` (hold, the default, keeps each line up until the next starts so captions connect). Saved values apply to every future video; `--max-words` still overrides `words` for a single run.
